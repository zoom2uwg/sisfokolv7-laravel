...
@section('title', 'Akademik — Edit Kelas')
...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between pb-5 border-b border-slate-800">
        <div>
            <h1 class="text-2xl font-bold bg-gradient-to-r from-indigo-200 to-purple-200 bg-clip-text text-transparent">Edit Kelas</h1>
            <p class="text-sm text-slate-400 mt-1">Perbarui informasi kelas <span class="text-indigo-300 font-semibold">{{ $kelas->nama }}</span>.</p>
        </div>
    ...
    <div class="bg-slate-900 border border-slate-800/60 rounded-2xl p-6 shadow-xl">
        <form method="POST" action="{{ route('academic.kelas.update', $kelas) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $kelas->nama) }}" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('nama') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                    @error('nama')
                ...
                <div>
                    ...
                    <input type="number" name="tingkat" id="tingkat" value="{{ old('tingkat', $kelas->tingkat) }}" min="1" max="12" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('tingkat') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                    @error('tingkat')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="kapasitas" id="kapasitas" value="{{ old('kapasitas', $kelas->kapasitas) }}" min="1" max="100" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('kapasitas') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                    @error('kapasitas')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="branch_id" id="branch_id" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('branch_id') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                        ...
                        @foreach($branches as $branch)
                            <option value="{{ $branch->id }}" {{ old('branch_id', $kelas->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->nama }}</option>
                        @endforeach
                    </select>
                    @error('branch_id')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>

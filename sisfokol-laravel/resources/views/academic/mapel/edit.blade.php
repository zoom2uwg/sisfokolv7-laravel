...
@section('title', 'Akademik — Edit Mata Pelajaran')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between pb-5 border-b border-slate-800">
        <div>
            <h1 class="text-2xl font-bold bg-gradient-to-r from-indigo-200 to-purple-200 bg-clip-text text-transparent">Edit Mata Pelajaran: {{ $mapel->nama }}</h1>
            <p class="text-sm text-slate-400 mt-1">Perbarui informasi mata pelajaran sesuai kebutuhan.</p>
        </div>
    ...
    <div class="bg-slate-900 border border-slate-800/60 rounded-2xl p-6 shadow-xl">
        <form method="POST" action="{{ route('academic.mapel.update', $mapel) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="kode" id="kode" value="{{ old('kode', $mapel->kode) }}" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('kode') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                    @error('kode')
                ...
                <div>
                    ...
                    <select name="mapel_jenis_id" id="mapel_jenis_id" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('mapel_jenis_id') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                        ...
                        @foreach($jenisList as $jenis)
                            <option value="{{ $jenis->id }}" {{ old('mapel_jenis_id', $mapel->mapel_jenis_id) == $jenis->id ? 'selected' : '' }}>{{ $jenis->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $mapel->nama) }}" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('nama') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                    @error('nama')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="jenjang" id="jenjang" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('jenjang') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                        ...
                        <option value="SMP" {{ old('jenjang', $mapel->jenjang) === 'SMP' ? 'selected' : '' }}>SMP</option>
                        <option value="SMA" {{ old('jenjang', $mapel->jenjang) === 'SMA' ? 'selected' : '' }}>SMA</option>
                        <option value="SMK" {{ old('jenjang', $mapel->jenjang) === 'SMK' ? 'selected' : '' }}>SMK</option>
                    </select>
                    @error('jenjang')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="kkm" id="kkm" value="{{ old('kkm', $mapel->kkm ?? 70) }}" min="0" max="100" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('kkm') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                    @error('kkm')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>

...
@section('title', 'Akademik — Edit Siswa')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between pb-5 border-b border-slate-800">
        <div>
            <h1 class="text-2xl font-bold bg-gradient-to-r from-indigo-200 to-purple-200 bg-clip-text text-transparent">Edit Data Siswa: {{ $siswa->nama }}</h1>
            <p class="text-sm text-slate-400 mt-1">Perbarui informasi profil siswa sesuai kebutuhan.</p>
        </div>
    ...
    <div class="bg-slate-900 border border-slate-800/60 rounded-2xl p-6 shadow-xl">
        <form method="POST" action="{{ route('academic.siswa.update', $siswa) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="nis" id="nis" value="{{ old('nis', $siswa->nis) }}" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('nis') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                    @error('nis')
                ...
                <div>
                    ...
                    <input type="text" name="nisn" id="nisn" value="{{ old('nisn', $siswa->nisn) }}" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('nisn') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                    @error('nisn')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $siswa->nama) }}" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('nama') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                    @error('nama')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="tempat_lahir" id="tempat_lahir" value="{{ old('tempat_lahir', $siswa->tempat_lahir) }}" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('tempat_lahir') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                    @error('tempat_lahir')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_lahir" id="tanggal_lahir" value="{{ old('tanggal_lahir', $siswa->tanggal_lahir ? $siswa->tanggal_lahir->format('Y-m-d') : '') }}" @fieldAttr('siswa.tanggal_lahir') class="w-full px-4 py-2.5 bg-slate-950/50 border @error('tanggal_lahir') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                    @error('tanggal_lahir')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                @field('siswa.telepon')
                    <div>
                        ...
                        <input type="text" name="telepon" id="telepon" value="{{ old('telepon', $siswa->telepon) }}" @fieldAttr('siswa.telepon') class="w-full px-4 py-2.5 bg-slate-950/50 border @error('telepon') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                        @error('telepon')
                            <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full px-4 py-2.5 bg-slate-950/50 border @error('status') border-rose-500 @else border-slate-800 @enderror rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm">
                        @foreach(['aktif' => 'Aktif', 'nonaktif' => 'Nonaktif', 'lulus' => 'Lulus', 'pindah' => 'Pindah', 'keluar' => 'Keluar'] as $val => $lbl)
                            <option value="{{ $val }}" {{ old('status', $siswa->status) === $val ? 'selected' : '' }}>{{ $lbl }}</option>
                        @endforeach
                    </select>
                    @error('status')
                        <p class="text-xs text-rose-500 mt-1.5">{{ $message }}</p>
